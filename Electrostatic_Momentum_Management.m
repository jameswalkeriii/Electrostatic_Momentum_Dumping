...
% Parameters for SSL-1300 the acting target/debris
    ...
    debris_D_MI = [1000; 1000; 1000].*eye(3)*1000000000; % [kg m2] SSL Moment of Inertia From email with Dan
...
plotting_servicer = params.servicer.N_spheres;
        for i = 1:size(params.servicer.N_spheres,2)
            ...
            new_sph_loc_serv = data_anti.C2*sph_loc;
            plotting_servicer(1:3,i) = new_sph_loc_serv;
        end
        
        
[ ~, ~, ~, N_L2_anti, ~, overlapFlag] = ...
    multisphereFT( params.debris.N_spheres, plotting_servicer, params.N_rvec_km*1000, params.V,...
    eye(3), eye(3),params.debris.D_COM, data_anti.C2'*params.servicer.S_COM);
        
% data_anti.N_L_elect_serv = data_anti.C2*data_anti.N_L_elect_serv;
serv_N_COM = params.servicer.S_COM;
...
[0 0 0], serv_N_COM + params.N_rvec_km*1000, [params.debris.voltage, params.servicer.voltage])

quiver3(params.servicer.S_COM(1) + params.N_rvec_km(1)*1000,params.servicer.S_COM(2) + params.N_rvec_km(2)*1000,params.servicer.S_COM(3) + params.N_rvec_km(3)*1000,...
